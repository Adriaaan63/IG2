...
public:
    EmisoresParticulas(Vector3 pos, SceneNode* node, SceneManager* sceneManager, int n_particle_system, int _type)
        ...
        if (type == 0) {
            ParticleSystem* pSys = mSM->createParticleSystem("pslaberintoSmoke" + to_string(n_particle_system), "laberintoSmoke");
            pSys->setEmitting(true);
            
            mNode->attachObject(pSys);
        }
        else if (type == 1) {
            ParticleSystem* pSys = mSM->createParticleSystem("psfire" + to_string(n_particle_system), "fireCinematica");
            pSys->setEmitting(true);
            mNode->attachObject(pSys);
        }
    }
